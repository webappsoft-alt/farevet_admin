...
import { ArrowLeft, Check, MinusCircle, PlusCircle, Trash2, X } from 'react-feather';
...
import { cameradark, spotlight1, spotlight10, spotlight11, spotlight12, spotlight13, spotlight14, spotlight15, spotlight16, spotlight17, spotlight18, spotlight19, spotlight2, spotlight20, spotlight21, spotlight22, spotlight23, spotlight24, spotlight25, spotlight26, spotlight27, spotlight28, spotlight29, spotlight3, spotlight30, spotlight31, spotlight32, spotlight33, spotlight34, spotlight35, spotlight36, spotlight37, spotlight38, spotlight39, spotlight4, spotlight5, spotlight6, spotlight7, spotlight8, spotlight9 } from '../../icons/icon';
const Option = Select
...
const CreateBusiness = () => {
    ...
    const serviceData = state?.businessData || null;
    ...
    const [multipleBusinesses, setMultipleBusinesses] = useState(
        [{ address: '', lat: null, lng: null, description: '', startDate: '', endDate: '' }]
    );
    ...
    const handleSwitchChange = (index) => {
        ...
        let availability = daysOfWeek;
        setSelectedDays(availability?.filter((val) => {
            return val?.checked !== false;
        }))
    };

    const handleStartTime = (value, index) => {
        const parsedTime = moment(value, 'h:mm A');
        const formattedValue = parsedTime.format('h:mm A');
    ...
    useEffect(() => {
        if (serviceData) {
            const serviceDataImages =
                serviceData?.business_images &&
                JSON.parse(serviceData?.business_images);
            setLoadBusinessImages(serviceDataImages || []);
            setSelectedCategories(serviceData?.cat_name)
            // Update payment methods
            const pMethods =
                serviceData?.payment_types &&
                JSON.parse(serviceData?.payment_types);
            setSelectPayment(pMethods || ['Credit / Debit']);

            // Update spotlight
            const spotLight =
                serviceData?.spotlight &&
                JSON.parse(serviceData?.spotlight);
            ...
            const gallery =
                serviceData?.gallery &&
                JSON.parse(serviceData?.gallery);
            setLoadGalleryImage(gallery || []);
            const timing =
                serviceData?.availability ?
                    JSON.parse(serviceData?.availability) : '';
            setSelectedDays(timing || '');
            setbusinessType(serviceData?.business_type)
            const updatedTiming = timing && timing?.map((day) => {
            ...
            const mergedDaysOfWeek = initialState && initialState?.map((initialDay) => {
                const existingDay = updatedTiming && updatedTiming?.find((day) => day.day === initialDay.day);
            ...
            const portfolio =
                ...
                JSON.parse(serviceData?.portfolio);
            setPortfolioArray(portfolio || [{ image: null, name: '', job_title: '' }]);
            ...
            // Update team members
            const members =
                serviceData?.team_members &&
                JSON.parse(serviceData?.team_members);
            setTeamMembers(members || [{ image: null, name: '', job_title: '' }]);
            if (!members || members.length === 0) {
                setTeamMembers([{}]);
            }
            const multipleBusiness =
                serviceData?.mobile_vet_detail &&
                JSON.parse(serviceData?.mobile_vet_detail);
            setMultipleBusinesses(multipleBusiness || [{ image: null, name: '', job_title: '' }]);
    ...
    const handleAddPortfolioItem = () => {
        setPortfolioArray([...portfolioArray, { image: null, name: '', job_title: '' }]);
    };
    ...
    const handleFileChangeTeam = async (index, id, e) => {
        ...
        try {
            ...
            setTeamMembers((prevMembers) => {
                ...
                newMembers[index] = { ...newMembers[index], image: response?.file_name };
                return newMembers;
            });
        } catch (error) {
            console.log(error);
    ...
    const toggleSpotlight = (title) => {
        if (selectSpotlight.includes(title)) {
            setSelectSpotlight(selectSpotlight.filter(selectedTitle => selectedTitle !== title));
    ...
    const handleTimeChange = (index, type, value) => {
        const updatedDaysOfWeek = [...daysOfWeek];
        updatedDaysOfWeek[index][type] = value;
        setDaysOfWeek(updatedDaysOfWeek);
    };
    ...
    const handlePhoneChange = (value, country) => {
        // const phoneNumberWithoutCode = value.replace(new RegExp(`^\\+${ country.dialCode }`), '').trim();
        setPhone(value);
        setCountryCode(country.dialCode);
        // console.log('Phone:', phoneNumberWithoutCode);
        // console.log('Coun/try Code:', country.dialCode);
    };
    ...
    const addBusiness = () => {
        setMultipleBusinesses(prevBusinesses => [
            ...prevBusinesses,
            { address: '', lat: '', lng: '', description: '', startDate: '', endDate: '' }
        ]);
    };
    ...
    const handleSubmit = async (values) => {
        ...
        body.append("mobile_vet_detail", businessType === 'mobile' ? (multipleBusinesses.length > 0 ? JSON.stringify(multipleBusinesses) : '') : '');
        body.append("address", businessType === 'standard' ? (locationDetails?.currentLocation ? locationDetails?.currentLocation : serviceData?.address) : multipleBusinesses[0]?.address);
        body.append("lat", businessType === 'standard' ? (locationDetails?.latitude ? locationDetails?.latitude : serviceData?.lat) : multipleBusinesses[0]?.lat);
        body.append("lng", businessType === 'standard' ? (locationDetails?.longitude ? locationDetails?.longitude : serviceData?.lng) : multipleBusinesses[0]?.lng);
        body.append("phone", phone);
        body.append("cat_name", selectedCategories);
        body.append("business_images", loadBusinessImages.length > 0 ? JSON.stringify(loadBusinessImages) : '');
        body.append("spotlight", JSON.stringify(selectSpotlight));
        body.append("gallery", loadGalleryImage.length > 0 ? JSON.stringify(loadGalleryImage) : '');
        body.append("portfolio", portfolio.length > 0 ? JSON.stringify(portfolio) : '');
        body.append("status", '1');
        body.append("business_created", 'admin');
        body.append("logo", loadBusinessFileLogo ? loadBusinessFileLogo : serviceData?.logo);
        body.append("team_members", teamMember.length > 0 ? JSON.stringify(teamMember) : '');
    ...
    return (
        <main className='container m-auto min-h-screen py-4'>
            <div className="flex items-center gap-3 mb-4">
                <button onClick={() => { navigate('/business') }} className="flex items-center justify-center w-[36px] h-[36px] bg_primary rounded-lg">
                    <ArrowLeft className='text_white' />
                </button>
                <span className="inter_semibold text-xl md:text-2xl text_dark">Create New Business</span>
            </div>
            <Form
                form={form}
                initialValues={{
            ...
            >
                    ...
                    <div className="w-full md:w-[70%] flex flex-wrap gap-2 items-center">
                        {business_type.map((item, i) => (
                            <button
                                ...
                                className={`border cursor-pointer rounded-3 gap-1 px-[1rem] py-2 inter_medium text-sm flex justify-center items-center ${businessType === item?.value
                ...
                {businessType === 'mobile' && multipleBusinesses.map((business, index) => (
                    <div key={index} className="flex gap-3 mb-5 w-full max-md:flex-col justify-start">
                        <span className="inter_medium text-sm text_dark w-full md:w-[30%]">
                            {index === 0 && (
                                <>
                                    Select Addresses
                        ...
                        <div key={index} className="w-full md:w-[70%]">
                            ...
                            <Col span={24} className='mb-3'>
                                <Autocomplete
                                    ...
                                    onPlaceSelected={(place) => {
                                        ...
                                        const updatedBusiness = { ...business, address, lat: latitude, lng: longitude };
                            ...
                            <Col span={24} className='mb-3'>
                                <InputStrap
                                    ...
                                    onChange={(e) => {
                                        const updatedBusiness = { ...business, description: e.target.value };
                                        handleBusinessUpdate(index, updatedBusiness);
                                    }}
                                />
                            </Col>
                            ...
                            <Row gutter={16} className='flex items-center'>
                                <Col span={10}>
                                    <InputStrap
                                        ...
                                        onChange={(e) => {
                                            const updatedBusiness = { ...business, startDate: e.target.value };
                                            handleBusinessUpdate(index, updatedBusiness);
                                        }}
                                    />
                                </Col>
                                <Col span={10}>
                                    <InputStrap
                                        ...
                                        onChange={(e) => {
                                            const updatedBusiness = { ...business, endDate: e.target.value };
                                            handleBusinessUpdate(index, updatedBusiness);
                                        }}
                                    />
                                </Col>
                                <Col span={4}>
                                    {index <= 0 ?
                                        ...
                                        :
                                        <button type='button' onClick={() => removeBusiness(index)}>
                                            <Trash2 style={{ color: '#FF6F61' }} className='text-[#FF6F61]' />
                                        </button>}
                                </Col>
                            </Row>
                        </div>
                        ...
                        >
                            <div>
                                <label htmlFor="fileInput" className="cursor-pointer">
                                    {businessFileLogo ? (
                                        <img src={businessFileLogo?.fileURL} alt="Preview" style={{ width: '120px', height: '120px' }} className="rounded-full object-cover" />
                                    ) : (
                                        <div style={{ width: '120px', height: '100px' }} className="rounded-lg flex justify-center items-center">
                                            {serviceData?.logo ? (
                                                <img src={`${global.IMAGEURL}/${serviceData?.logo}`} alt="" className="rounded-full object-cover" style={{ width: '120px', height: '120px' }} />
                                            ) : (
                        ...
                        >
                            <div className="flex gap-3 items-center flex-wrap">
                                <label htmlFor="fileInput2" className="cursor-pointer">
                                    <div style={{ width: '120px', height: '100px' }} className="border rounded-lg flex justify-center items-center">
                                        <img src={cameradark} alt="Camera Icon" />
                                    </div>
                                </label>
                ...
                <div className="flex gap-3 mb-4 w-full max-md:flex-col justify-start">
                    ...
                    <div className='w-full md:w-[70%] flex flex-wrap gap-2 items-center'>
                        ...
                        {/* {selectedServices.map((serviceId) => (
                            <span key={serviceId} className='bg-primary text-white rounded-lg px-2 py-1 mr-2'>
                    ...
                    <div className='w-full md:w-[70%] flex flex-wrap gap-2 items-center'>
                        {spotlight.map((item, i) => (
                            <button
                                key={i}
                                type='button'
                                className={`border cursor-pointer rounded-5 gap-1 px-3 py-2 inter_medium text-sm flex justify-center items-center ${selectSpotlight.includes(item?.title) ? 'bg-[#F8F2FD] text_primary' : 'bg_white text_secondary'}`}
                                onClick={() => toggleSpotlight(item?.title)}>
                                {selectSpotlight.includes(item?.title) && (
                                    <Check className='text_primary' size={16} />
                                )}
                                <span className="flex items-center gap-1">
                                    <img src={item?.icon} alt={item?.title} style={{ height: '20px', width: 'auto' }} className="w-4 h-4 object-cover" />
                                    {item?.title}
                                </span>
                            </button>
                        ))}
                                    ...
                                    <div className="w-full flex-wrap flex-md-nowrap gap-2 flex items-center">
                                        <div>
                                            <InputStrap
                                                ...
                                                // value={daysOfWeek?.from}
                                                value={moment(daysOfWeek[index].from, 'h:mm A').format('HH:mm')}
                                                id="time"
                                            />
                                        </div>
                                        ...
                                        <div>
                                            <InputStrap
                                                required
                                                type="time"
                                                value={moment(daysOfWeek[index].to, 'h:mm A').format('HH:mm')}
                            ...
                            <div className='w-full md:w-[70%] relative'>
                                <label htmlFor={`fileInput-${index}`} className="cursor-pointer">
                                    {portfolioFile && portfolioFile[`fileInput-${index}`] ? (
                                        <div>
                                            <img style={{ width: '120px', height: '100px' }} src={portfolioFile[`fileInput-${index}`]?.fileURL} alt="" className="rounded-lg object-cover mx-auto" />
                                        </div>
                                    ) : (portfolioItem?.image ? (
                                        <img style={{ width: '120px', height: '100px', borderRadius: '20px' }} src={`${global.IMAGEURL}/${portfolioItem?.image}`} alt="Camera Icon" />
                                    ) : (
                                        <div style={{ width: '120px', height: '100px' }} className="border  flex justify-center items-center rounded-lg object-cover mx-auto">
                                            <img src={cameradark} alt="Camera Icon" />
                                        </div>)
                                    )}
                                </label>
                                <Input
                                    ...
                                    id={`fileInput-${index}`}
                                    className="visually-hidden"
                                    onChange={(e) => handleFileChangePortfolio(index, `fileInput-${index}`, e)}
                                />
                            </div>
                        </div>
                        <div className="flex gap-3 mb-4 w-full max-md:flex-col justify-start">
                            ...
                            <div className='w-full md:w-[70%]'>
                                <InputStrap
                                    ...
                                    onChange={(e) => handlePortfolioNameChange(index, e.target.value)}
                            ...
                            <div className='w-full md:w-[70%]'>
                                {selectedCategories === 'Veterinary' ?
                                    <Select
                                        ...
                                        onChange={(value) => handlePortfolioTitleChange(index, value)}
                                    >
                ...
                {teamMembers.map((teamMember, index) => (
                    <div key={index} className="flex gap-3 mb-4 w-full max-md:flex-col justify-start">
                        <span className="inter_medium text-sm text_dark w-full md:w-[30%]">
                            {index === 0 && (
                                <>
                                    Team Members
                        ...
                        <div key={index} className="w-full md:w-[70%]">
                            <Row gutter={16} className='flex items-center w-full'>
                                <Col xs={4} md={2}>
                                    <label htmlFor={`fileInput_${index}`} className="cursor-pointer">
                                        {teamFile && teamFile[`fileInput_${index}`] ? (
                                            <div>
                                                <img style={{ width: '2.5rem', height: '2.5rem' }} src={teamFile[`fileInput_${index}`]?.fileURL} alt="" className="rounded-full aspect-square object-cover" />
                                            </div>
                                        ) : (teamMember?.image ? (
                                            <div>
                                                <img style={{ width: '2.5rem', height: '2.5rem' }} className="rounded-full aspect-square object-cover" src={`${global.IMAGEURL}/${teamMember?.image}`} alt="Camera Icon" />
                                            </div>
                                        ) : (
                                            <div style={{ width: '2.5rem', height: '2.5rem' }} className="border rounded-full flex items-center justify-center object-cover">
                                                <img src={cameradark} alt="Camera Icon" />
                                            </div>)
                                        )}
                                    </label>
                                    <Input
                                        ...
                                        id={`fileInput_${index}`}
                                        className="visually-hidden"
                                        onChange={(e) => handleFileChangeTeam(index, `fileInput_${index}`, e)}
                                    />
                                </Col>
                                <Col span={9}>
                                    <InputStrap
                                ...
                                <Col span={3}>
                                    {index <= 0 ?
                                        ...
                                        :
                                        <button type='button' onClick={() => handleDeleteTeamMember(index)}>
                                            <Trash2 style={{ color: '#FF6F61' }} className='text-[#FF6F61]' />
                                        </button>}
                                </Col>
                            </Row>
                ...
                <div className="flex justify-end w-full my-3">
                    {!isProcessing ? (
                        <button type='submit' className="flex justify-center bg_primary py-[12px] px-[1rem] rounded-lg items-center button_shadow">
                            <span className="inter_semibold text-sm text_white">Create Business</span>
                        </button>
                    ) : (
                        <button type="button" className='flex justify-center bg_primary py-[12px] px-[4rem] rounded-lg items-center button_shadow' disabled>
