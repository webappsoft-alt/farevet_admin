...
import { Col, Form, Input, Row, Select, Switch, TreeSelect, message } from 'antd';
...
import { ArrowLeft, Check, MinusCircle, Plus, PlusCircle, Trash2, X } from 'react-feather';
...
const UpdateBusiness = () => {
    ...
    const handleStartTime = (value, index) => {
        ...
        const formattedValue = parsedTime.isValid() ? parsedTime.format('HH:mm') : ''; // Format to 24-hour if valid, otherwise set empty string
    ...
    const handleSubmit = async (values) => {
        ...
        body.append("availability", selectedDays ? JSON.stringify(selectedDays || '') : '');
        await apiRequest({ body })
            .then(async (res) => {
    ...
    return (
        <main className='container m-auto min-h-screen py-4'>
            <div className="flex items-center gap-3 mb-4">
                ...
                <span className="inter_semibold text-xl md:text-2xl text_dark">Update Business</span>
            </div>
            <Form form={form}
                initialValues={{
                        ...
                        {spotlightArray.map((item, i) => (
                            ...
                            >
                                ...
                                <span className="flex items-center gap-1">
                                    <img
                                        src={spotlightArray.find(spot => spot?.title === item?.title)?.icon}
                ...
                <div className="flex justify-end w-full my-3">
                    {!isProcessing ? (
                        <button type='submit' className="flex justify-center bg_primary py-[12px] px-[1rem] rounded-lg items-center button_shadow">
                            <span className="inter_semibold text-sm text_white">Update Business</span>
                        </button>
                    ) : (
