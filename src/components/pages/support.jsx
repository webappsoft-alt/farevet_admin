...
const Support = () => {
    ...
    const handleFetchSupport = async () => {
        ...
        try {
            ...
            body.append("table_name", 'support');
    ...
    const columns = [
        {
            ...
            sortable: true,
            minWidth: '150px',
            selector: row => row?.user?.name || 'User'
        },
        {
            name: 'Email',
            sortable: true,
        ...
        },
        {
            ...
            cell: (row) => {
                return (
                    <p className="flex w-full plusJakara_regular items-center">
                        {row?.title}
                    </p>
                )
            }
            ...
            cell: (row) => {
                return (
                    <p className="flex w-full mb-0 plusJakara_regular items-center">
                        {moment(row?.created_by).format('DD-MM-YYYY')}
                    </p>
                )
            }
        },
            ...
            cell: (row) => {
                return (
                    <div className="flex w-full gap-2 items-center">
                        ...
                        >
                            {row?.status === 'processing' ? 'Query in Processing' : row?.status === 'treated' ? 'Query Being Treated' : 'Query Completed'}
                        </button>}
                    </div>
                )
            }
    ...
    const handleUpdateSupport = async (status, business) => {
        setsupportStatus(status)
        setsupportId(business?.id)
        setLoading2(true)
        try {
            const body = new FormData();
            body.append('type', 'update_data');
        ...
        <StyleSheetManager shouldForwardProp={(prop) => !['sortActive'].includes(prop)}>
            <main className='container m-auto height_calc flex-grow flex flex-col p-3'>
                <div className="flex w-full mb-4">
                    <span className="text_dark plusJakara_medium text-2xl md:text-3xl">Customer Support</span>
                </div>
                <ProductTable
