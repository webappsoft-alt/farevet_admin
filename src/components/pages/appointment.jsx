...
const Appointments = () => {
  ...
  const handleShowDetail = (item) => {
    ...
    setShowDetail(true);
    const availability = item?.business?.availability && JSON.parse(item?.business?.availability) || [];
  ...
  const checkIsDayExist = day => {
    ...
    if (dayInfo) {
      ...
      message.error('The store is closed on the selected date. Kindly select another date or update availability')
    }
  };
    ...
    if (selectedItem?.status === 'pending') {
      return (
        <>
          ...
          >
            {isProcessing2 && orderStatus === 'processing' ? <CircularProgress color="inherit" size={18} /> : 'Approve'}
          </button>
          <button
          ...
          >
            {isProcessing2 && orderStatus === 'cancelled' ? <CircularProgress color="inherit" size={18} /> : 'Decline'}
          </button>
        </>
      );
    ...
    <main className="container m-auto height_calc flex-grow flex flex-col p-3">
      ...
      <div className="flex items-center flex-wrap w-fit border rounded-xl my-4 gap-y-2">
        <button
          ...
          className={`flex justify-center inter_medium max-lg:text-xs text-sm items-center py-2 px-2 px-sm-3 ${selectedOption === "all"
        ...
        <button
          ...
          className={`flex justify-center inter_medium max-lg:text-xs text-sm items-center py-2 px-2 px-sm-3 ${selectedOption === "pending"
            ? "bg_primary text_white rounded-lg"
            : "text_dark"
            } `}
        >
        ...
        <button
          ...
          className={`flex justify-center inter_medium max-lg:text-xs text-sm items-center py-2 px-2 px-sm-3 ${selectedOption === "processing"
            ? "bg_primary text_white rounded-lg"
            : "text_dark"
            } `}
        >
        ...
        <button
          ...
          className={`flex justify-center inter_medium max-lg:text-xs text-sm items-center py-2 px-2 px-sm-3 ${selectedOption === "completed"
            ? "bg_primary text_white rounded-lg"
            : "text_dark"
            } `}
        >
        ...
        <button
          ...
          className={`flex justify-center inter_medium max-lg:text-xs text-sm items-center py-2 px-2 px-sm-3 ${selectedOption === "cancelled"
            ? "bg_primary text_white rounded-lg"
        ...
        <div className="d-flex flex-wrap gap-3 mb-4 justify-content-center justify-content-lg-start">
          {!categories || categories?.length === 0 ?
            <div className="my-5 flex justify-center items-center w-full">
              <span className="text_dark inter_medium">No Appointment Found</span>
            </div>
            : (
            ...
            <div className="flex flex-col my-3 items-start">
              ...
              <div className="flex my-3 flex-col gap-2 w-full">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col items-start">
                    <span className="text_dark inter_medium">{selectedDeal?.business?.name}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div
      ...
      <Modal show={showSheduleModal} onHide={() => setShowSheduleModal(false)} centered>
        <Modal.Header closeButton>
          <div className="flex justify-start">
            <span className="text_dark text-2xl plusJakara_medium">Reschedule</span>
          </div>
        </Modal.Header>
        <Modal.Body>
          <Form layout="verticle" onFinish={handleSubmit}>
            <div className="flex gap-2 items-center mb-2">
              <button
                ...
                style={{ backgroundColor: selectShedule === 'date' ? '#06d6a0' : '#f6f6f6', color: selectShedule === 'date' ? '#fff' : '#d3d3d3' }}
              ...
              <button
                ...
                style={{ backgroundColor: selectShedule === 'time' ? '#06d6a0' : '#f6f6f6', color: selectShedule === 'time' ? '#fff' : '#d3d3d3' }}
                className={`w-1/2 rounded-lg inter_semibold flex items-center justify-center p-2`}
              >
                Time
              </button>
                  ...
                  >
                    {timeSlots?.map((item, i) => (
                      <SwiperSlide
                        ...
                        className={`py-2 text-center rounded-2 border ${time === item ? 'bg_primary text_white' : 'text_primary bg_white'} `}>{item}</SwiperSlide>
                    ))}
                  </Swiper>
                </div>
              ) : (
                <div className="my-4 flex flex-nowrap scrolbar2 px-2 gap-2" style={{ overflowX: 'auto' }}>
                  <Input
                    required
                    type="date"
                    value={date}
            ...
            >
              {isProcessing3 ? <CircularProgress size={18} color="inherit" /> : 'Reschedule'}
            </button>
          </Form>
        </Modal.Body>
      </Modal >
            ...
            <Modal.Body>
              <Form onFinish={handleUpdate}>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      <div className="flex items-center gap-1">
                        <img src={weight} alt="" />
                        <span className="text-sm plusJakara_medium text_dark">
                          {selectedItem?.pet?.avg_weight ? selectedItem?.pet?.avg_weight + ' lbs' :
                ...
                <div className="flex flex-col mb-2">
                  ...
                  <div className="d-flex flex-wrap gap-1 align-items-center">
                    {selectedItem?.order_type === "deal" &&
                      !selectedItem?.deal?.deal_services || selectedItem?.deal?.deal_services === null ? <span>Not Found</span> :
                          ...
                          <div className="ml-2">
                            {service?.sub_service &&
                              JSON.parse(service.sub_service).map((subService, j) => (
                                <span key={j} className="text_secondary text-sm plusJakara_regular">
                                  {subService}
                                  {j < JSON.parse(service.sub_service).length - 1 && ", "}
                                </span>
                              ))}
                          </div>
                        </div>
                ...
                <div className="flex justify-between w-full items-center flex-wrap gap-2 mb-2">
                  <div className="flex gap-2 items-center flex-wrap">
                    <button
                      type="button"
                      onClick={() => selectedItem?.status === 'pending' && handleSheduleModal()}
